...
export const getPartnerSection=asyncHandler(async(req:Request, res:Response)=>{
    const partnerSection= await PartnerSection.findOne();
...
export const updatePartnerSection=asyncHandler(async(req:Request, res:Response)=>{
    ...
    if (badges !==undefined){
        ...
        if(!Array.isArray(parsedBadges)){
            throw new ApiError(400,"Badges must be an array of string")
        }
...
export const updateAffiliation = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!id) {
        throw new ApiError(400, "Affiliation id is required");
    }
    ...
    if (!existing) {
        throw new ApiError(404, "Affiliation item not found");
    }
    ...
    if (req.file) {
        const uploadedLogo = await uploadImageToCloud(req.file, "affiliations");
        affiliationUpdate["affiliations.$.logo"] = uploadedLogo.url;
        affiliationUpdate["affiliations.$.logoPublicId"] = uploadedLogo.publicId;

        const currentLogo = existing.affiliations.find(
            (a) => a._id?.toString() === id
        )?.logoPublicId;

        if (currentLogo) {
            await deleteFromCloud(currentLogo, "image");
        }
    }
...
export const deleteAffiliation = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (targetAffiliation.logoPublicId) {
        await deleteFromCloud(targetAffiliation.logoPublicId, "image");
    }
